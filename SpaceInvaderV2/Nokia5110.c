...
#define DC            (*((volatile uint32_t *)0x40005008))  // PB1
#define DC_COMMAND    0x00
#define DC_DATA       0x02

#define RESET         (*((volatile uint32_t *)0x40005004))  // PB0
...
void static lcdwrite(enum typeOfWrite type, char message){
	...
	if(type == COMMAND){
		...
		while((SSI2_SR_R & SSI_SR_TNF) ==0);//wait until done transmitting
		DC = DC_DATA;
...
void Nokia5110_Init(void){
	...
	SYSCTL_RCGCSSI_R |= SSI2;
	SYSCTL_RCGCGPIO_R |= PORTB;
	while((SYSCTL_RCGCGPIO_R & PORTB)!=PORTB);
	
	delay = SYSCTL_RCGCGPIO_R;
	
	// Configure Port B 
	GPIO_PORTB_DEN_R   |= B_PINS;       // Enable digital PB4,5,7
	GPIO_PORTB_DEN_R   |= LCD_GPIO;     // Enable digital PB0, PB1

	GPIO_PORTB_DIR_R   |= B_PINS;       // PB4,5,7 as output (SSI pins)
	GPIO_PORTB_DIR_R   |= LCD_GPIO;     // PB0, PB1 as output (RESET, DC)

	GPIO_PORTB_AMSEL_R &= ~B_PINS;      // Disable analog on PB4,5,7
	GPIO_PORTB_AMSEL_R &= ~LCD_GPIO;    // Disable analog on PB0, PB1

	GPIO_PORTB_AFSEL_R |=  B_PINS;      // PB4, PB5, PB7  SSI2
	GPIO_PORTB_AFSEL_R &= ~LCD_GPIO;    // PB0, PB1 GPIO

	// Clear PB0, PB1 PCTL fields (GPIO)
	GPIO_PORTB_PCTL_R &= ~0x000000FF;

	// Configure PB4, PB5, PB7 for SSI2 function
	GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R & 0x0000FFFF) | 0x20220000;
	...
	SSI2_CR0_R &=~ (SSI_CR0_SCR_M | SSI_CR0_SPH | SSI_CR0_SPO);
